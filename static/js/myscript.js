...
$("#loginbtn").on("click", function(){
  ...
  $.post("/login", {username: user, password: password}, function (response) {
    ...
    if (response.success) {
      ...
      $("#loader").html('<p class="alert alert-danger">Invalid username/password <span id="clear-msg" style="cursor: pointer;">&times;</span></p>');
...
$.post("/signup", {username: user, password: password}, function (response) {
  ...
  if (response.success) {
    window.location.href = nextURL;
     ...
     $("#loader").html('<p class="alert alert-danger">Email is invalid or already taken <span id="clear-msg" style="cursor: pointer;">&times;</span></p>');
     $("#clear-msg").on("click", function() {
      $("#loader").html('');
    });
  }
...
$("#addGallerybtn").on("click", function(){
  ...
  var password = $("#galleryPassword").val();

  if(galleryName != "") {

    $.when($.post("/galleries/add", {galleryName: galleryName, password: password}).done(function(response){
          $("#loader").html('');
          var response = JSON.parse(response);
